...
{
    public class TwoHalfOpt : OptimisationMethod
    {
            ...
            while (!locallyOptimal)
            {
                ...
                for(int counter_1 = 0; counter_1 <= N - 3; counter_1++)
                {
                    ...
                    for(int counter_2 = counter_1 + 2; counter_2 <= N - 1; counter_2++)
                    {
                        ...
                        var gain = TwoOpt.GainFrom2Opt(X1, X2, Y1, Y2);
        ...
        private double GainFromNodeShift(City X0_pred, City X0, City X0_succ, City Y1, City Y2)
        {
            var del_Length = X0_pred.CostTo(X0) + X0.CostTo(X0_succ) + Y1.CostTo(Y2);
            var add_Length = X0_pred.CostTo(X0_succ) + Y1.CostTo(X0) + X0.CostTo(Y2);
